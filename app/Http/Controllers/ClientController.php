...
class ClientController extends Controller
{
    ...
    public function processClientProfile(Request $request)
    {
        if ($request->session()->has('uid')) {
            ...
            if ($validator->fails()) {
                ...
                return redirect('client_dashboard')->with(
                    'msg',
                    'Successfully Saved'
                );
            }
    ...
    {
        if (
            Setting::where('setting_key', 'allow_self_registration')->first()
                ->setting_value == 1
        ) {
            $rules = [
            ...
            if ($validator->fails()) {
                ...
                $borrower->files = serialize([]);
                $borrower->working_status = $request->working_status;
                if (
                    Setting::where(
                        'setting_key',
                        'client_auto_activate_account'
                ...
                if ($borrower->active == 1) {
                    ...
                    Flash::success(
                        trans('general.successfully_registered_logged_in')
                    );
                    return redirect('client')->with(
                        'msg',
                        trans('general.logged_in')
                    );
                }
                Flash::success(trans('general.successfully_registered'));
                return redirect('client')->with(
                    'msg',
                    trans('general.successfully_registered')
                );
            }
        } else {
    ...
    {
        $data = LoanApplication::where(
            'borrower_id',
            $request->session()->has('uid')
        )->get();
        $borrower = Borrower::find($request->session()->get('uid'));
    ...
    public function createApplication(Request $request)
    {
        ...
        foreach (LoanProduct::all() as $key) {
            $products[$key->id] =
                $key->name .
                '(' .
                round($key->minimum_principal) .
                '-' .
                round($key->maximum_principal) .
                ')';
        }
        $branches = [];
        ...
        return view(
            'client.apply',
            compact('borrower', 'products', 'branches')
        );
    }
    ...
    public function showApplication(Request $request, $loan_application)
    {
        ...
        $borrower = Borrower::find($request->session()->get('uid'));
        return view(
            'client.show_application',
            compact('loan_application', 'borrower')
        );
    }
    ...
    public function createGuarantor(Request $request, $loan_application)
    {
        ...
        return view(
            'client.create_guarantor',
            compact('loan_application', 'borrower', 'borrowers')
        );
    }
    ...
    public function storeGuarantor(Request $request, $loan_application)
    {
        ...
        Flash::success(trans_choice('general.successfully_saved', 1));
        return redirect(
            'client/application/' . $loan_application->id . '/show'
        );
    }
    ...
    {
        $custom_fields = CustomFieldMeta::where('category', 'savings')
            ->where('parent_id', $saving->id)
            ->get();
        $pdf = PDF::loadView(
            'saving.pdf_statement',
            compact('saving', 'custom_fields', 'transactions')
        );
        return $pdf->download(
            $saving->borrower->title .
                ' ' .
                $saving->borrower->first_name .
                ' ' .
                $saving->borrower->last_name .
    ...
    public function paySaving(Request $request, $saving)
    {
        ...
        $methods = [];
        if (
            Setting::where('setting_key', 'paypal_enabled')->first()
                ->setting_value == 1 &&
            !empty(
                Setting::where('setting_key', 'paypal_email')->first()
                    ->setting_value
            )
        ) {
        ...
        if (
            Setting::where('setting_key', 'paynow_enabled')->first()
                ->setting_value == 1 &&
            !empty(
                Setting::where('setting_key', 'paynow_key')->first()
                    ->setting_value
            ) &&
            !empty(
                Setting::where('setting_key', 'paynow_id')->first()
                    ->setting_value
            )
        ) {
        ...
        if (
            Setting::where('setting_key', 'stripe_enabled')->first()
                ->setting_value == 1 &&
            !empty(
                Setting::where('setting_key', 'stripe_secret_key')->first()
                    ->setting_value
            ) &&
            !empty(
                Setting::where('setting_key', 'stripe_publishable_key')->first()
                    ->setting_value
            )
        ) {
        ...
        if (
            Setting::where('setting_key', 'mpesa_enabled')->first()
                ->setting_value == 1 &&
            !empty(
                Setting::where('setting_key', 'mpesa_consumer_key')->first()
                    ->setting_value
            ) &&
            !empty(
                Setting::where('setting_key', 'mpesa_consumer_secret')->first()
                    ->setting_value
            ) &&
            !empty(
                Setting::where('setting_key', 'mpesa_shortcode')->first()
                    ->setting_value
            ) &&
            !empty(
                Setting::where('setting_key', 'mpesa_endpoint')->first()
                    ->setting_value
            )
        ) {
        ...
        return view(
            'client.pay_saving',
            compact('borrower', 'saving', 'methods')
        );
    }
    ...
    {
        $values = [
            'id' => Setting::where('setting_key', 'paynow_id')->first()
                ->setting_value,
            'reference' => $saving->id,
            'amount' => $request->amount,
            'returnurl' => url(
                'client/saving/' . $saving->id . '/pay/paynow/return'
            ),
            'resulturl' => url(
                'client/saving/' . $saving->id . '/pay/paynow/result'
            ),
            'status' => 'Message',
        ];
        //generate hash
        ...
        $integrationkey = Setting::where('setting_key', 'paynow_key')->first()
            ->setting_value;
    ...
    public function paynowReturnSaving(Request $request, $saving)
    {
        ...
        if (
            $status == 'Paid' ||
            ($status = 'Awaiting Delivery' || ($status = 'Delivered'))
        ) {
    ...
    public function paynowResultSaving(Request $request, $saving)
    {
        ...
        if (
            ...
            if (!empty($saving->savings_product->chart_reference)) {
                ...
                $journal->account_id =
                    $saving->savings_product->chart_reference->id;
            ...
            if (!empty($saving->savings_product->chart_control)) {
                $journal = new JournalEntry();
                $journal->account_id =
                    $saving->savings_product->chart_control->id;
                $journal->branch_id = $savings_transaction->branch_id;
                $journal->date = date('Y-m-d');
                $journal->year = $date[0];
                $journal->month = $date[1];
    ...
    {
        $stripe = [
            'secret_key' => Setting::where(
                'setting_key',
                'stripe_secret_key'
            )->first()->setting_value,
            'publishable_key' => Setting::where(
                'setting_key',
            ...
            while (!feof($fp)) {
                ...
                if (strcmp($res, 'VERIFIED') == 0) {
                    ...
                    if (
                        ...
                        $savings_transaction = new SavingTransaction();
                        $savings_transaction->borrower_id =
                            $saving->borrower_id;
    ...
    public function showLoan(Request $request, $loan)
    {
        ...
        $schedules = LoanSchedule::where('loan_id', $loan->id)
            ...
            ->get();
        $custom_fields = CustomFieldMeta::where('category', 'loans')
    ...
    public function pay(Request $request, $loan)
    {
        ...
        $due_items = GeneralHelper::loan_due_items(
            $loan->id,
            $loan->release_date,
            date('Y-m-d')
        );
        $paid_items = GeneralHelper::loan_paid_items(
            $loan->id,
            $loan->release_date,
            date('Y-m-d')
        );
        ...
        return view(
            'client.pay',
            compact('borrower', 'loan', 'due', 'methods')
        );
    }
    ...
    public function paynow(Request $request, $loan)
    {
        $values = [
            ...
            'amount' => $request->amount,
            'returnurl' => url(
                'client/loan/' . $loan->id . '/pay/paynow/return'
            ),
            'resulturl' => url(
                'client/loan/' . $loan->id . '/pay/paynow/result'
            ),
            'status' => 'Message',
        ];
        //generate hash
    ...
    public function paynowResult(Request $request, $loan)
    {
        ...
        if (
            ...
            $allocation = GeneralHelper::loan_allocate_payment(
                $loan_transaction
            );
            //return $allocation;
            //principal
            if ($allocation['principal'] > 0) {
                if (!empty($loan->loan_product->chart_loan_portfolio)) {
                    $journal = new JournalEntry();
                    $journal->account_id =
                        $loan->loan_product->chart_loan_portfolio->id;
                ...
                if (!empty($loan->loan_product->chart_fund_source)) {
                    $journal = new JournalEntry();
                    $journal->account_id =
                        $loan->loan_product->chart_fund_source->id;
                    $journal->branch_id = $loan->branch_id;
                    $journal->date = date('Y-m-d');
                    $journal->year = $date[0];
                    $journal->month = $date[1];
            ...
            if ($allocation['interest'] > 0) {
                if (!empty($loan->loan_product->chart_income_interest)) {
                    $journal = new JournalEntry();
                    $journal->account_id =
                        $loan->loan_product->chart_income_interest->id;
                    $journal->branch_id = $loan->branch_id;
                    $journal->date = date('Y-m-d');
                    $journal->year = $date[0];
                    $journal->month = $date[1];
                ...
                if (!empty($loan->loan_product->chart_receivable_interest)) {
                    $journal = new JournalEntry();
                    $journal->account_id =
                        $loan->loan_product->chart_receivable_interest->id;
                    $journal->branch_id = $loan->branch_id;
                    $journal->date = date('Y-m-d');
                    $journal->year = $date[0];
                    $journal->month = $date[1];
            ...
            if ($allocation['fees'] > 0) {
                if (!empty($loan->loan_product->chart_income_fee)) {
                    $journal = new JournalEntry();
                    $journal->account_id =
                        $loan->loan_product->chart_income_fee->id;
                    $journal->branch_id = $loan->branch_id;
                    $journal->date = date('Y-m-d');
                    $journal->year = $date[0];
                    $journal->month = $date[1];
                ...
                if (!empty($loan->loan_product->chart_receivable_fee)) {
                    $journal = new JournalEntry();
                    $journal->account_id =
                        $loan->loan_product->chart_receivable_fee->id;
                    $journal->branch_id = $loan->branch_id;
                    $journal->date = date('Y-m-d');
                    $journal->year = $date[0];
                    $journal->month = $date[1];
            ...
            if ($allocation['penalty'] > 0) {
                if (!empty($loan->loan_product->chart_income_penalty)) {
                    $journal = new JournalEntry();
                    $journal->account_id =
                        $loan->loan_product->chart_income_penalty->id;
                    $journal->branch_id = $loan->branch_id;
                    $journal->date = date('Y-m-d');
                    $journal->year = $date[0];
                    $journal->month = $date[1];
                ...
                if (!empty($loan->loan_product->chart_receivable_penalty)) {
                    $journal = new JournalEntry();
                    $journal->account_id =
                        $loan->loan_product->chart_receivable_penalty->id;
                    $journal->branch_id = $loan->branch_id;
                    $journal->date = date('Y-m-d');
                    $journal->year = $date[0];
                    $journal->month = $date[1];
                ...
                if (strcmp($res, 'VERIFIED') == 0) {
                    ...
                    if (
                        ...
                        if ($allocation['principal'] > 0) {
                            if (
                                !empty(
                                    $loan->loan_product->chart_loan_portfolio
                                )
                            ) {
                                $journal = new JournalEntry();
                                $journal->account_id =
                                ...
                                $journal->borrower_id = $loan->borrower_id;
                                $journal->transaction_type = 'repayment';
                                $journal->transaction_sub_type =
                                    'repayment_principal';
                                $journal->name = 'Principal Repayment';
                                $journal->loan_id = $loan->id;
                                $journal->loan_transaction_id =
                                    $loan_transaction->id;
                                $journal->credit = $allocation['principal'];
                                $journal->reference = $loan_transaction->id;
                                $journal->save();
                            ...
                            if (
                                !empty($loan->loan_product->chart_fund_source)
                            ) {
                                $journal = new JournalEntry();
                                $journal->account_id =
                                    $loan->loan_product->chart_fund_source->id;
                        ...
                        if ($allocation['interest'] > 0) {
                            if (
                                !empty(
                                    $loan->loan_product->chart_income_interest
                                )
                            ) {
                                $journal = new JournalEntry();
                                $journal->account_id =
                        ...
                        if ($allocation['fees'] > 0) {
                            ...
                            if (
                                !empty(
                                    $loan->loan_product->chart_receivable_fee
                                )
                            ) {
                                $journal = new JournalEntry();
                                $journal->account_id =
                        ...
                        if ($allocation['penalty'] > 0) {
                            if (
                                !empty(
                                    $loan->loan_product->chart_income_penalty
                                )
                            ) {
                                $journal = new JournalEntry();
                                $journal->account_id =
                        ...
                        if (
                            round(
                                GeneralHelper::loan_total_balance($loan->id)
                            ) <= 0
                        ) {
                            $l = Loan::find($loan->id);
                            $l->status = 'closed';
    ...
    public function mpesa(Request $request, $loan)
    {
        ...
        $url =
            Setting::where('setting_key', 'mpesa_endpoint')->first()
                ->setting_value . '/mpesa/c2b/v1/registerurl';
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_HTTPHEADER, [
            'Content-Type:application/json',
        ...
        $url =
            Setting::where('setting_key', 'mpesa_endpoint')->first()
                ->setting_value . '/mpesa/c2b/v1/simulate';
    ...
    public function validate_mpesa_loan()
    {
        try {
            ...
            $resp =
                '{"ResultCode":0,"ResultDesc":"Validation passed successfully"}';
            ...
            $resp =
                '{"ResultCode": 1, "ResultDesc":"Validation failure due to internal service error"}';
        }
        echo $resp;
    }
    ...
    public function confirm_mpesa_loan()
    {
        try {
            ...
            header('Content-Type:application/json');
            $resp =
                '{"ResultCode":0,"ResultDesc":"Confirmation recieved successfully"}';
            //read incoming request
            $postData = file_get_contents('php://input');
    ...
    {
        $pdf = PDF::loadView(
            'loan.pdf_loan_statement',
            compact('loan', 'payments')
        );
        $pdf->setPaper('A4', 'landscape');
        return $pdf->download(
            $loan->borrower->title .
