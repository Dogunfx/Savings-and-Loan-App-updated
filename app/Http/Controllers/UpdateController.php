...
class UpdateController extends Controller
{
    ...
    public function update_2_0_3()
    {
        ...
        foreach (Loan::whereIn('loans.status',
            ...
            if (!empty($key->loan_product)) {
                //payments
                foreach (LoanRepayment::where('loan_id', $key->id)->orderBy('collection_date',
                    'asc')->get() as $repayment) {
    ...
    public function import_expense_income()
    {
        //fix schedules
        $count = 0;
        //first empty Journal Entries table
        JournalEntry::where('transaction_type', 'expense')->delete();
        JournalEntry::where('transaction_type', 'income')->delete();
        JournalEntry::where('savings_id', '!=', null)->delete();
        //import loan transactions and journals
        $count = 0;
        foreach (Expense::get() as $expense) {
            if (!empty($expense->chart)) {
                $count++;
                $journal = new JournalEntry();
                $journal->user_id = $expense->user_id;
        ...
        foreach (OtherIncome::get() as $other_income) {
            if (!empty($other_income->chart)) {
                $count++;
                $journal = new JournalEntry();
                $journal->user_id = $other_income->user_id;
        ...
        foreach (SavingTransaction::get() as $savings_transaction) {
            //make journal transactions
            $saving = $savings_transaction->savings;
            if (!empty($saving)) {
                $date = $savings_transaction->date;
                if ($savings_transaction->type == "deposit") {
                    if (!empty($saving->savings_product->chart_reference)) {
                        ...
                        $journal->date = $savings_transaction->date;
                        $journal->year = $date[0];
                        $journal->month = $date[1];
                        ...
                        $journal->debit = $savings_transaction->credit;
                    ...
                    if (!empty($saving->savings_product->chart_control)) {
                        ...
                        $journal->name = "Deposit";
                        $journal->savings_id = $saving->id;
                        $journal->credit = $savings_transaction->credit;
                        $journal->reference = $savings_transaction->id;
                        $journal->save();
                    }
                }
                if ($savings_transaction->type == "withdrawal") {
                    ...
                    if (!empty($saving->savings_product->chart_reference)) {
                        ...
                        $journal->savings_id = $saving->id;
                        $journal->credit = $savings_transaction->debit;
                        $journal->reference = $savings_transaction->id;
                        $journal->save();
                    }
                    if (!empty($saving->savings_product->chart_control)) {
                        ...
                        $journal->name = "Withdrawal";
                        $journal->savings_id = $saving->id;
                        $journal->debit = $savings_transaction->debit;
                        $journal->reference = $savings_transaction->id;
                        $journal->save();
                    }
                }
        ...
        Flash::success("Imported records");
        return redirect('dashboard');
    }
}
