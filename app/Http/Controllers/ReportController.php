...
class ReportController extends Controller
{
    ...
    public function expected_repayments(Request $request)
    {
        ...
        $start_date = $request->start_date;
        $end_date = $request->end_date;
        $loans_items = GeneralHelper::loans_items($start_date, $end_date);
        return view('loan_report.expected_repayments',
            compact('start_date',
                'end_date', 'loans_items'));
    }
    ...
    public function expected_repayments_excel(Request $request)
    {
        ...
        array_push($data, [
            ...
            number_format($loans_items["principal"], 2),
            number_format($loans_items["interest"], 2),
            number_format($loans_items["fees"], 2),
            number_format($loans_items["penalty"], 2),
            number_format($loans_items["principal"] + $loans_items["interest"] + $loans_items["fees"] + $loans_items["penalty"],
                2),
        ]);
        array_push($data, [
            trans_choice('general.actual', 1),
            number_format($loans_items["principal_paid"], 2),
            number_format($loans_items["interest_paid"], 2),
            number_format($loans_items["fees_paid"], 2),
            number_format($loans_items["penalty_paid"], 2),
            number_format($loans_items["principal_paid"] + $loans_items["interest_paid"] + $loans_items["fees_paid"] + $loans_items["penalty_paid"],
                2),
        ]);
        array_push($data, [
            trans_choice('general.balance', 1),
            number_format($loans_items["principal"] - $loans_items["principal_paid"], 2),
            number_format($loans_items["interest"] - $loans_items["interest_paid"], 2),
            number_format($loans_items["fees"] - $loans_items["fees_paid"], 2),
            number_format($loans_items["penalty"] - $loans_items["penalty_paid"], 2),
            number_format(($loans_items["principal"] + $loans_items["interest"] + $loans_items["fees"] + $loans_items["penalty"]) - ($loans_items["principal_paid"] + $loans_items["interest_paid"] + $loans_items["fees_paid"] + $loans_items["penalty_paid"]),
                2),
        ]);
    ...
    public function repayments_report_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            $total_interest = 0;
            $total_penalty = 0;
            foreach (DB::table("loan_transactions")->leftJoin("borrowers", "borrowers.id", "loan_transactions.borrower_id")->leftJoin("loan_repayment_methods", "loan_repayment_methods.id", "loan_transactions.repayment_method_id")->selectRaw("loan_transactions.*,borrowers.first_name first_name,borrowers.last_name last_name,loan_repayment_methods.name repayment_method")->where('loan_transactions.transaction_type',
                'repayment')->where('loan_transactions.reversed', 0)->where('loan_transactions.branch_id',
                session('branch_id'))->when($start_date, function ($query) use ($start_date, $end_date) {

            })->get() as $key) {
                $principal = $key->principal_paid;
                $interest = $key->interest_paid;
                $fees = $key->fees_paid;
                $penalty = $key->penalty_paid;
